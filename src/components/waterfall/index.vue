<template>
  <div>
    ...
    <slot v-if="isOver" name="footer">
      <div class="wf-over-message">到底了！</div>
    </slot>
  </div>
</template>
...
import { getDevice } from "@/utils/mobile";
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    const { colWidth, gap, mobileGap, list, isLoading, isOver, isMounted } =
      toRefs(props);
    ...
    const { actualColWidth, actualCols, colsTop, calculateActualCols } =
    ...
    const wrapperWidth = computed(() => {
      return (
        actualColWidth.value * actualCols.value +
        actualGap * (actualCols.value - 1)
      );
    });
    // 加载状态
    ...
    watch(isMounted, (newV: boolean) => {
      if (scrollBodySelector && newV) {
        ...
        scrollElement = body = document.querySelector(
          scrollBodySelector
        ) as HTMLElement;
...
.wf-item {
  ...
  position: absolute;
  transition: all 0.3s;
  animation: scaleItem 0.2s linear;
}
.wf-over-message {
  height: 40px;
  line-height: 40px;
